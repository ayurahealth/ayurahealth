...
function renderMarkdown(text: string, doshaColor = '#6abf8a'): string {
  return text
    ...
    .replace(/^### (.+)$/gm, `<h3 style="color:${doshaColor};font-size:1rem;font-weight:600;margin:1rem 0 0.4rem;font-family:\'Cormorant Garamond\',serif">$1</h3>`)
    .replace(/^## (.+)$/gm, `<h2 style="color:${doshaColor};font-size:1.1rem;font-weight:600;margin:1rem 0 0.4rem;font-family:\'Cormorant Garamond\',serif">$1</h2>`)
...
export default function ChatPage() {
  ...
  useEffect(() => {
    const SRClass = (window as Window & { SpeechRecognition?: typeof SpeechRecognition; webkitSpeechRecognition?: typeof SpeechRecognition }).SpeechRecognition || (window as Window & { webkitSpeechRecognition?: typeof SpeechRecognition }).webkitSpeechRecognition
    if (SRClass && window.speechSynthesis) setVoiceSupported(true)
  }, [])
  ...
  const startChat = useCallback((d?: Dosha | null) => {
    ...
    const greeting = meta
      ? tx.greeting_dosha
          ...
          .replace('{tagline}', tx[meta.taglineKey as keyof typeof tx] as string)
          .replace('{desc}', tx[meta.descKey as keyof typeof tx] as string)
      : tx.greeting
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#05100a', fontFamily: '"DM Sans", system-ui, sans-serif', color: '#e8dfc8', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: '-20%', right: '-10%', width: '60vw', height: '60vw', background: 'radial-gradient(circle, rgba(201,168,76,0.04) 0%, transparent 70%)', borderRadius: '50%' }} />
      </div>
        ...
        <div style={{ display: 'flex', gap: '0.35rem', alignItems: 'center' }}>
          {LANG_OPTIONS.map(opt => (
            <button key={opt.code} onClick={() => setLang(opt.code)} style={{
              padding: '0.25rem 0.5rem', borderRadius: 20, fontSize: '0.7rem',
              border: `1px solid ${lang === opt.code ? 'rgba(106,191,138,0.5)' : 'rgba(255,255,255,0.1)'}`,
              background: lang === opt.code ? 'rgba(106,191,138,0.12)' : 'transparent',
              color: lang === opt.code ? '#6abf8a' : 'rgba(200,200,200,0.5)',
              cursor: 'pointer', fontWeight: lang === opt.code ? 600 : 400,
              transition: 'all 0.2s',
            }}>
              {opt.flag} {opt.label}
            </button>
          ...
          {dosha && screen === 'chat' && (
            <div style={{
              ...
              border: `1px solid ${DOSHA_META[dosha].color}40`,
              background: DOSHA_META[dosha].bg, color: DOSHA_META[dosha].color,
            }}>
          ...
          <button onClick={() => setIncognito(!incognito)} style={{
            ...
            border: `1px solid ${incognito ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.1)'}`,
            background: incognito ? 'rgba(255,255,255,0.08)' : 'transparent',
            color: incognito ? '#fff' : 'rgba(200,200,200,0.5)',
      ...
      {screen === 'landing' && (
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 600, margin: '0 auto', padding: '3rem 1.5rem 6rem' }}>
          <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
            ...
            <p style={{ color: 'rgba(232,223,200,0.65)', fontSize: '1.05rem', lineHeight: 1.7, maxWidth: 480, margin: '0 auto 2rem' }}>
              {tx.hero_sub}
            </p>
          </div>
          ...
          <div style={{
            ...
            marginBottom: '1rem',
            boxShadow: '0 0 40px rgba(106,191,138,0.06)',
          }}>
            <div style={{ fontSize: '2rem', textAlign: 'center', marginBottom: '0.75rem' }}>🧬</div>
            <h3 style={{ fontFamily: '"Cormorant Garamond", serif', color: '#c9a84c', fontSize: '1.4rem', textAlign: 'center', fontWeight: 600, marginBottom: '0.5rem' }}>
              {tx.quiz_cta_title}
            </h3>
            <p style={{ color: 'rgba(232,223,200,0.55)', fontSize: '0.875rem', textAlign: 'center', lineHeight: 1.6, marginBottom: '1.5rem' }}>
              {tx.quiz_cta_sub}
            </p>
            <button onClick={() => { setCurrentQ(0); setAnswers([]); setScreen('quiz') }} style={{
              width: '100%', padding: '0.9rem',
              background: 'linear-gradient(135deg, #2d5a1b, #3d7a28)',
              color: '#f0e6c8', border: 'none', borderRadius: 14,
          ...
          <button onClick={() => { setDosha(null); startChat(null) }} style={{
            width: '100%', padding: '0.8rem',
            background: 'transparent', color: 'rgba(200,200,200,0.45)',
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem', marginTop: '2.5rem', flexWrap: 'wrap' }}>
            {[
              { emoji: '📜', text: 'Charaka Samhita' },
              { emoji: '☯️', text: 'Huangdi Neijing' },
              { emoji: '🏔️', text: 'Sowa Rigpa' },
              { emoji: '💊', text: 'Evidence-Based' },
            ].map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: 'rgba(200,200,200,0.3)', fontSize: '0.75rem' }}>
                <span>{s.emoji}</span><span>{s.text}</span>
              </div>
            ))}
          </div>

          <p style={{ textAlign: 'center', color: 'rgba(200,200,200,0.25)', fontSize: '0.68rem', marginTop: '2rem', lineHeight: 1.5 }}>
            {tx.disclaimer}
          </p>
        </div>
      )}
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 560, margin: '0 auto', padding: '2rem 1.5rem 6rem' }}>
          <div style={{ marginBottom: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
              <span style={{ color: 'rgba(200,200,200,0.4)', fontSize: '0.8rem', letterSpacing: '0.05em' }}>{tx.question_of} {currentQ + 1} {tx.of} {questions.length}</span>
              <span style={{ color: 'rgba(200,200,200,0.4)', fontSize: '0.8rem' }}>{Math.round((currentQ / questions.length) * 100)}%</span>
            </div>
            <div style={{ height: 3, background: 'rgba(255,255,255,0.07)', borderRadius: 2 }}>
              <div style={{ height: 3, background: 'linear-gradient(90deg, #3d7a28, #6abf8a)', borderRadius: 2, width: `${(currentQ / questions.length) * 100}%`, transition: 'width 0.4s ease', boxShadow: '0 0 8px rgba(106,191,138,0.5)' }} />
            </div>
          </div>

          <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
            <div style={{ fontSize: '3rem', marginBottom: '1rem', filter: 'drop-shadow(0 0 15px rgba(106,191,138,0.3))' }}>{questions[currentQ].emoji}</div>
            <h2 style={{ fontFamily: '"Cormorant Garamond", serif', color: '#e8dfc8', fontSize: '1.5rem', fontWeight: 600, lineHeight: 1.4 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {questions[currentQ].opts.map((opt, i) => (
              <button key={i} onClick={() => handleAnswer(opt.d)} style={{
                padding: '1rem 1.25rem',
                background: 'rgba(255,255,255,0.03)',
                border: '1px solid rgba(106,191,138,0.15)',
                borderRadius: 16, color: 'rgba(232,223,200,0.85)',
                fontSize: '0.95rem', cursor: 'pointer', textAlign: 'left', lineHeight: 1.5,
          ...
          {currentQ > 0 && (
            <button onClick={() => { setCurrentQ(currentQ - 1); setAnswers(answers.slice(0, -1)) }} style={{
              marginTop: '1.5rem', background: 'none', border: 'none',
              color: 'rgba(200,200,200,0.35)', fontSize: '0.85rem', cursor: 'pointer',
            }}>
              {tx.back}
            </button>
          )}
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 540, margin: '0 auto', padding: '2rem 1.5rem 6rem' }}>
          <div style={{ opacity: revealed ? 1 : 0, transform: revealed ? 'translateY(0)' : 'translateY(24px)', transition: 'all 0.6s ease' }}>
            <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
              <div style={{ fontSize: '5rem', marginBottom: '0.5rem', filter: `drop-shadow(0 0 30px ${DOSHA_META[dosha].glow})`, animation: 'floatEmoji 3s ease-in-out infinite' }}>
                {DOSHA_META[dosha].emoji}
              </div>
              <div style={{ color: 'rgba(200,200,200,0.4)', fontSize: '0.85rem', marginBottom: '0.4rem', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                {tx.your_dosha_is}
              </div>
              <h2 style={{
                fontFamily: '"Cormorant Garamond", serif',
                fontSize: '3.5rem', fontWeight: 700,
                color: DOSHA_META[dosha].color,
                textShadow: `0 0 40px ${DOSHA_META[dosha].glow}`,
                margin: 0,
              }}>
                {dosha}
              </h2>
              <p style={{ color: 'rgba(200,200,200,0.5)', fontStyle: 'italic', marginTop: '0.5rem', fontSize: '1rem' }}>
                {tx[DOSHA_META[dosha].taglineKey as keyof typeof tx] as string}
              </p>
            </div>

            <div style={{
              background: DOSHA_META[dosha].bg,
              border: `1px solid ${DOSHA_META[dosha].color}30`,
              borderRadius: 20, padding: '1.5rem',
              marginBottom: '1.25rem',
              boxShadow: `0 0 40px ${DOSHA_META[dosha].glow}20`,
            }}>
              <p style={{ color: 'rgba(232,223,200,0.75)', fontSize: '0.95rem', lineHeight: 1.7, marginBottom: '1.25rem' }}>
                {tx[DOSHA_META[dosha].descKey as keyof typeof tx] as string}
              </p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                {[
                  { label: tx.strengths, key: DOSHA_META[dosha].strengthsKey },
                  { label: tx.watch, key: DOSHA_META[dosha].watchKey },
                ].map((item, i) => (
                  <div key={i}>
                    <div style={{ color: DOSHA_META[dosha].color, fontSize: '0.7rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '0.4rem' }}>
                      {item.label}
                    </div>
                    <div style={{ color: 'rgba(232,223,200,0.65)', fontSize: '0.85rem', lineHeight: 1.5 }}>
            ...
            <button onClick={() => startChat(dosha)} style={{
              ...
              background: `linear-gradient(135deg, ${DOSHA_META[dosha].color}90, ${DOSHA_META[dosha].color}60)`,
              color: '#fff', border: `1px solid ${DOSHA_META[dosha].color}50`,
              borderRadius: 16, fontSize: '1rem', fontWeight: 600,
              cursor: 'pointer', marginBottom: '0.75rem',
              boxShadow: `0 4px 20px ${DOSHA_META[dosha].glow}`,
              transition: 'all 0.2s',
            }}>
              {tx.start_consult}
            </button>
      ...
      {screen === 'chat' && (
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 760, margin: '0 auto', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 57px)' }}>
          ...
          <div style={{
            ...
            borderBottom: '1px solid rgba(106,191,138,0.08)',
            background: 'rgba(5,16,10,0.6)',
            backdropFilter: 'blur(10px)',
          }}>
            {MEDICINE_SYSTEMS.map(sys => (
              <button key={sys.id} onClick={() => {
                setSelectedSystems(prev => prev.includes(sys.id) ? prev.filter(s => s !== sys.id) : [...prev, sys.id])
              }} style={{
                padding: '0.22rem 0.65rem', borderRadius: 20,
                border: `1px solid ${selectedSystems.includes(sys.id) ? 'rgba(106,191,138,0.4)' : 'rgba(255,255,255,0.07)'}`,
                background: selectedSystems.includes(sys.id) ? 'rgba(106,191,138,0.1)' : 'transparent',
                color: selectedSystems.includes(sys.id) ? '#6abf8a' : 'rgba(200,200,200,0.35)',
                fontSize: '0.7rem', cursor: 'pointer', transition: 'all 0.15s',
                fontWeight: selectedSystems.includes(sys.id) ? 500 : 400,
              }}>
                {sys.label}
              </button>
            ))}
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '1.25rem 1rem 0.5rem' }}>
              ...
              <div key={i} style={{
                ...
                {msg.role === 'assistant' && (
                  <div style={{
                    ...
                    background: 'linear-gradient(135deg, #2d5a1b, #4a9e6a)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: '0.8rem', boxShadow: '0 0 10px rgba(106,191,138,0.3)',
                  }}>🌿</div>
                )}
                <div style={{ maxWidth: '82%' }}>
                  <div style={{
                    ...
                    background: msg.role === 'user'
                      ? 'linear-gradient(135deg, rgba(45,90,27,0.7), rgba(60,110,35,0.6))'
                      : 'rgba(255,255,255,0.04)',
                    border: msg.role === 'user'
                      ? '1px solid rgba(106,191,138,0.25)'
                      : '1px solid rgba(106,191,138,0.1)',
                    fontSize: '0.9rem', lineHeight: 1.7,
                    color: msg.role === 'user' ? '#f0e6c8' : 'rgba(232,223,200,0.85)',
                    backdropFilter: 'blur(10px)',
                    boxShadow: msg.role === 'assistant' ? '0 4px 20px rgba(0,0,0,0.2)' : 'none',
                  }}>
                    {msg.role === 'assistant'
                      ? <div dangerouslySetInnerHTML={{ __html: renderMarkdown(msg.content, doshaColor) }} />
                      : msg.content}
                  </div>
                  {msg.role === 'assistant' && voiceSupported && (
                    <button onClick={() => speakText(msg.content)} style={{
                      marginTop: '0.4rem', background: 'none', border: 'none',
                      color: isSpeaking ? '#6abf8a' : 'rgba(200,200,200,0.25)',
            ...
            {streaming && (
              <div style={{ marginBottom: '1.25rem', display: 'flex', alignItems: 'flex-end', gap: '0.5rem' }}>
                ...
                <div style={{
                  maxWidth: '82%', padding: '1rem 1.25rem',
                  ...
                  <span style={{ opacity: 0.4, animation: 'pulse 1s ease infinite' }}>▋</span>
                </div>
              </div>
            )}
            ...
            {loading && !streaming && (
              <div style={{ marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'linear-gradient(135deg, #2d5a1b, #4a9e6a)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.8rem', animation: 'pulse 1.5s ease infinite' }}>🌿</div>
          ...
          <div style={{
            ...
            background: 'rgba(5,16,10,0.85)',
            backdropFilter: 'blur(20px)',
            borderTop: '1px solid rgba(106,191,138,0.08)',
          }}>
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-end', maxWidth: 700, margin: '0 auto' }}>
              ...
              {voiceSupported && (
                <button onClick={startListening} style={{
                  ...
                  background: isListening ? 'rgba(232,131,90,0.2)' : 'rgba(106,191,138,0.08)',
                  border: `1px solid ${isListening ? 'rgba(232,131,90,0.5)' : 'rgba(106,191,138,0.2)'}`,
                  color: isListening ? '#e8835a' : '#6abf8a',
              ...
              <textarea
                ...
                rows={1}
                style={{
                  flex: 1, padding: '0.75rem 1rem', borderRadius: 22,
                  border: '1px solid rgba(106,191,138,0.15)',
                  background: 'rgba(255,255,255,0.04)',
                  backdropFilter: 'blur(10px)',
                  color: '#e8dfc8',
                ...
                onFocus={e => e.target.style.borderColor = 'rgba(106,191,138,0.35)'}
                onBlur={e => e.target.style.borderColor = 'rgba(106,191,138,0.15)'}
              />
              ...
              <button onClick={() => sendMessage()} disabled={loading || !input.trim()} style={{
                ...
                background: loading || !input.trim()
                  ...
                  : 'linear-gradient(135deg, #2d5a1b, #4a9e6a)',
                border: 'none',
                color: loading || !input.trim() ? 'rgba(255,255,255,0.2)' : '#fff',
                cursor: loading || !input.trim() ? 'not-allowed' : 'pointer',
                fontSize: '1.1rem',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                boxShadow: !loading && input.trim() ? '0 0 15px rgba(106,191,138,0.3)' : 'none',
                transition: 'all 0.2s',
              }}>
                ↑
              </button>
            </div>
            <p style={{ textAlign: 'center', color: 'rgba(200,200,200,0.2)', fontSize: '0.62rem', marginTop: '0.4rem', letterSpacing: '0.03em' }}>
              {tx.edu_disclaimer}
            </p>
          </div>
        </div>
