  ...
  const pointsBuffer = useMemo(() => {
    ...
    for (let i = 0; i < BIOMARKER_MAP.length; i++) {
      for (let j = i + 1; j < BIOMARKER_MAP.length; j++) {
        // ⚡ Bolt: Push primitive coordinates directly to avoid intermediate Vector3 objects
        p.push(...BIOMARKER_MAP[i].position)
        p.push(...BIOMARKER_MAP[j].position)
      }
    }
    // ⚡ Bolt: Fully memoize the typed array to prevent WebGL buffer reallocation on every render
