  ...
  const positions = useMemo(() => {
    ...
    for (let i = 0; i < BIOMARKER_MAP.length; i++) {
      for (let j = i + 1; j < BIOMARKER_MAP.length; j++) {
        // ⚡ Bolt: Avoid intermediate THREE.Vector3 objects and directly push primitives.
        p.push(...BIOMARKER_MAP[i].position)
        p.push(...BIOMARKER_MAP[j].position)
      }
    }
    // ⚡ Bolt: Memoize the typed array to prevent WebGL buffer reallocation on every render.
