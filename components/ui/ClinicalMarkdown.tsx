...
const ClinicalMarkdown = React.memo(function ClinicalMarkdown({
  ...
  const components = React.useMemo(() => ({
    ...
    thead: ({ children }: any) => <thead style={{ background: 'var(--surface-mid)', borderBottom: '1px solid var(--border-low)' }}>{children}</thead>,
    th: ({ children }: any) => <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontWeight: 700, color: 'var(--text-main)' }}>{children}</th>,
    td: ({ children }: any) => <td style={{ padding: '0.75rem 1rem', borderTop: '1px solid var(--border-low)', color: 'var(--text-muted)' }}>{children}</td>,
  }), []);
