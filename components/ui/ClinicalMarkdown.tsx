...
export default function ClinicalMarkdown({ 
  ...
  // Optimization: Memoize the components map to prevent ReactMarkdown from completely unmounting
  // and remounting the entire DOM tree on every render (which causes severe jank during streaming).
  const markdownComponents = React.useMemo(() => ({
    ...
    thead: ({ children }: React.HTMLAttributes<HTMLTableSectionElement>) => <thead style={{ background: 'var(--surface-mid)', borderBottom: '1px solid var(--border-low)' }}>{children}</thead>,
    th: ({ children }: React.ThHTMLAttributes<HTMLTableCellElement>) => <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontWeight: 700, color: 'var(--text-main)' }}>{children}</th>,
    td: ({ children }: React.TdHTMLAttributes<HTMLTableCellElement>) => <td style={{ padding: '0.75rem 1rem', borderTop: '1px solid var(--border-low)', color: 'var(--text-muted)' }}>{children}</td>,
  }), [])
